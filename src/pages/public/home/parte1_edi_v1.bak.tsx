...
// ΓöÇΓöÇ Part├¡culas Sutiles (Estrellas m├ís visibles y con resplandor) ΓöÇΓöÇ
function GoldenParticles() {
  ...
  useEffect(() => {
    // Generamos m├ís estrellas (50), algunas con m├ís brillo que otras
    const newParticles = Array.from({ length: 50 }).map((_, i) => ({
      id: i,
      x: Math.random() * 100,
      y: Math.random() * 100,
      size: Math.random() * 2 + 1, // Tama├▒o entre 1px y 3px
      duration: Math.random() * 15 + 15,
      delay: Math.random() * 5,
      isBright: Math.random() > 0.5 // 50% de probabilidad de brillar m├ís fuerte
    }));
...
// ΓöÇΓöÇ Typewriter Principal (Titular) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Progreso de scroll ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Carrusel Animado (Tipo M├íquina de Escribir) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const highlightPhrases = [
  { base: "Nos quedamos hasta el primer cierre contable operado en producci├│n. ", gold: "Por contrato." },
  { base: "Garantizamos la estabilizaci├│n total de los procesos cr├¡ticos. ", gold: "Cero desv├¡os." },
  { base: "Transici├│n a soporte sin incidencias bloqueantes abiertas. ", gold: "Sin sorpresas." }
];
...
function TypewriterCarousel() {
  ...
  useEffect(() => {
    ...
    // Si ya escribi├│ toda la frase, espera 2.5 segundos antes de empezar a borrar
    if (!isDeleting && charIdx === fullTextLength) {
      const pauseTimeout = setTimeout(() => setIsDeleting(true), 2500);
      return () => clearTimeout(pauseTimeout);
    }

    // Si ya borr├│ toda la frase, pasa a la siguiente
...
// ΓöÇΓöÇ Borde serpiente SVG (Ciclo Perfecto) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Avatares ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Chat IA ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
type Msg = { role: 'ai' | 'user'; text: string };

function HeroAIChat() {
  const [messages, setMessages] = useState<Msg[]>([
    { role: 'ai', text: 'Diagnostic Engine iniciado. ┬┐En qu├⌐ etapa se encuentra tu implementaci├│n Oracle Fusion?' },
  ]);
  ...
  const send = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setMessages(p => [...p, {
        role: 'ai',
        text: 'Analizando variables de riesgoΓÇª Detecto posible desviaci├│n en tu ciclo cr├¡tico. Recomiendo iniciar una sesi├│n t├⌐cnica con nuestro equipo.',
      }]);
    }, 1600);
  };
  ...
  return (
    <SnakeBorder>
      ...
      <form onSubmit={send} className="shrink-0 relative border-t border-[#181818] px-3 py-3" style={{ background: 'rgba(0,0,0,0.55)' }}>
        <input type="text" value={input} disabled={typing} onChange={e => setInput(e.target.value)} placeholder="Consulta de diagn├│stico..."
...
// ΓöÇΓöÇ COMPONENTE PRINCIPAL ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
export default function Parte1Home() {
  ...
  // Umbrales de desaparici├│n secuencial
  ...
  return (
    <div className="relative w-full flex flex-col items-center justify-center min-h-screen pb-12 lg:pb-16 overflow-hidden bg-[#0A0A0A] selection:bg-[#C9A96E]/30">
      ...
      <div className="relative z-10 w-full max-w-[1340px] px-6 md:px-12 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-10 items-center">
        ...
        {/* ΓöÇΓöÇ COLUMNA IZQUIERDA ΓöÇΓöÇ */}
        <div className="lg:col-span-7 flex flex-col items-start relative">
          ...
          <h1 className="mt-6 font-serif tracking-tight text-[#F5F5F5]">
            ...
            >
              ...
              <em className="not-italic text-[#C9A96E]">cr├¡tico opera.</em>
            </span>
          </h1>

          {/* ΓöÇΓöÇ P├üRRAFO Y M├üQUINA DE ESCRIBIR ΓöÇΓöÇ */}
          <div className={`mt-7 w-full max-w-[540px] transition-all duration-1000 delay-[700ms] ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
            ...
            {/* M├íquina de Escribir Rotativa */}
            <TypewriterCarousel />
          </div>

          {/* ΓöÇΓöÇ TAGS ANIMADOS ΓöÇΓöÇ */}
          <div className="mt-8 flex flex-wrap gap-3"
               style={{ opacity: fadeTags, transform: `translateY(${(1 - fadeTags) * -8}px)` }}>
            {['Rescate Fusion', 'Migraci├│n SAP/EBS', 'Greenfield Oracle'].map((tag, index) => (
              <span 
                key={tag} 
          ...
          {/* ΓöÇΓöÇ BOT├ôN DE DIAGN├ôSTICO ΓöÇΓöÇ */}
