...
export default function S06bFixedPrice() {
  ...
  return (
    <section id="fixed-price" ref={containerRef} className="parte8-section relative w-full bg-[#050505] overflow-hidden flex items-center justify-center border-t border-[#111] py-20 md:py-32">
      ...
      {/* GRADIENTES PARA FUSIONAR CON EL RESTO DE LA PÁGINA */}
      <div className="absolute inset-0 z-0 bg-gradient-to-t from-[#050505] via-transparent to-[#050505] pointer-events-none" />
      <div className="absolute inset-0 z-0 bg-gradient-to-r from-[#050505] via-transparent to-[#050505] pointer-events-none" />

      {/* 2. TARJETA DE GARANTÍA (OVERLAY) */}
      <div className={`relative z-10 w-full max-w-[900px] px-6 transition-all duration-1000 ease-[cubic-bezier(0.16,1,0.3,1)] ${isVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-16 scale-95'}`}>
        
        <div className="relative bg-[#0A0A0A]/70 backdrop-blur-xl border border-[#C9A96E]/40 p-10 md:p-16 shadow-[0_0_50px_rgba(0,0,0,0.8)] rounded-sm overflow-hidden group hover:border-[#C9A96E]/70 transition-colors duration-500">
          ...
          <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-[#C9A96E]/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700" />

          {/* Header de la tarjeta */}
          <div className="mb-10 flex items-center gap-4">
            <div className="h-px flex-1 bg-[#2A2A2A]" />
            <h3 className="font-mono text-[10px] md:text-xs tracking-[0.25em] text-[#C9A96E] uppercase font-bold">
              La Garantía FABRIC
            </h3>
            <div className="h-px flex-1 bg-[#2A2A2A]" />
          </div>
          ...
          <blockquote className="relative">
            <span className="absolute -top-6 -left-4 font-serif text-6xl text-[#C9A96E]/20 select-none">"</span>
            
            <p className="relative z-10 font-serif text-2xl md:text-3xl lg:text-4xl text-[#F5F5F5] italic leading-[1.4] md:leading-[1.5] mb-10 text-center">
              Si después de 90 días post go-live, tu Oracle Fusion sigue requiriendo reportes manuales ejecutivos paralelos por causa atribuible a FABRIC, <span className="text-[#C9A96E] not-italic font-medium">devolvemos el 100% de los honorarios</span> de la fase de estabilización.
            </p>
          </blockquote>

          {/* Footer del contrato */}
          <div className="flex flex-col md:flex-row items-center justify-between pt-8 border-t border-[#2A2A2A]/60 gap-4">
            <p className="font-sans text-sm md:text-base text-[#F5F5F5]/50 text-center md:text-left">
              Esto no es marketing. Es cláusula contractual estándar en cada proyecto FABRIC.
            </p>
            
            <div className="flex items-center gap-2">
              <span className="h-1 w-1 bg-[#C9A96E] rounded-full" />
